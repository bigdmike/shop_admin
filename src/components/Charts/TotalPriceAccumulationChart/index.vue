<template src="./template.html"></template>

<script>
import VueApexCharts from 'vue-apexcharts'
export default {
  name: "TotalPriceAccumulationChart",
  components: {
    VueApexCharts
  },
  props: {
    order_data: {
      require: true,
      type: Array
    },
    date_between: {
      require: true,
      type: Array
    }
  },
  data() {
    return {
      MoneyChartOptions: {
        chart: {
          id: 'vuechart-example'
        },
        xaxis: {
          categories: []
        }
      },
      MoneyChartSeries: [
        {
          name: '金額總計',
          data: []
        }
      ],
    }
  },
  methods: {

    SetChart() {
      var start = this.date_between[0];
      var end = this.date_between[1];
      var startTime = new Date(start);
      var endTime = new Date(end);

      let date_all = []
      while ((endTime.getTime() - startTime.getTime()) >= 0) {
        var month = (startTime.getMonth() + 1).toString().length == 1 ? "0" + (startTime.getMonth() + 1).toString() : (startTime.getMonth() + 1).toString();
        var day = startTime.getDate().toString().length == 1 ? "0" + startTime.getDate() : startTime.getDate();
        date_all.push(month + "-" + day);
        startTime.setDate(startTime.getDate() + 1);
      }
      this.MoneyChartOptions.xaxis.categories = date_all
      this.SetSeries()
    },
    SetSeries() {
      let data = []
      let total = 0
      this.MoneyChartOptions.xaxis.categories.forEach((date, date_index) => {
        data[date_index] = total
        this.order_data.forEach(item => {
          if (item.create_time.substring(5, 10) == date) {
            data[date_index] += parseInt(item.total_price)
          }
        });
        if (data[date_index] == total) {
          this.MoneyChartOptions.xaxis.categories[date_index] = " "
        }
        total = data[date_index]
      })
      this.MoneyChartSeries[0].data = data
    }
  },
  watch: {
    order_data() {
      this.order_data.length <= 0 ? "" : this.SetChart()
    },
    date_between() {
      this.SetChart()
    }
  },
  created() { },
  computed: {
    total_money() {
      return this.MoneyChartSeries[0].data[this.MoneyChartSeries[0].data.length - 1]
    }
  },
  filters: {
    formatPrice(value) {
      return value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
    }
  }
}
</script>