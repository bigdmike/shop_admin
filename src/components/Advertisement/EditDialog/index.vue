...
export default {
  ...
  methods: {
    Open(item) {
      this.id = item.AdvertisementID;
      this.title = item.Title;
      this.content = item.Content;
      this.category = item.AdvertisementCategoryID;
      this.seq = item.Seq;
      this.dialog = true;
      this.cover = null;
      if (item.Image1 != "") {
        this.image_preview_url =
          item.Image1.split("/")[item.Image1.split("/").length - 1];
      }
      this.errors = {
        title: "",
        content: "",
