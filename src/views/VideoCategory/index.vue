...
import EditDialog from '@/components/VideoCategory/CategoryEditDialog/';
...
export default {
  ...
  methods: {
    CheckSort() {
      let is_sort = true;
      this.video_categories.forEach((item, item_index) => {
    ...
    GetData() {
      get_categories().then((res) => {
        res.data.forEach((item, item_index) => {
          ...
          res.data[item_index].ID = item.VideoCategoryID;
        });
        this.video_categories = res.data;
        this.CheckSort();
      });
    },
